...
def sipoc_metrics_game():
    ...
    with st.expander("🌟 Why SIPOC Matters – Click to be Inspired"):
        st.markdown("""
...
Discover the real impact of your work on Customers and Colleagues.
    ...
    if st.button("🔍 Show All Correct Answers"):
        st.subheader("✅ Here are the correct answers:")
        solution_df = pd.DataFrame(results)
        st.dataframe(solution_df)

    if st.button("📊 View Full SIPOC & Metrics Overview"):
        st.subheader("☕ Make a Coffee – SIPOC Overview")
        st.table({
            "Supplier": ["Barista", "Grinder"],
            "Input": ["Ground Coffee", "Water", "Clean Cup"],
            "Process": ["Putting moka on stove", "Boiling water", "Lighting the stove"],
            "Output": ["Hot coffee in cup"],
            "Customer": ["Customer"]
        })
        st.markdown("### 📈 Metrics")
        st.table({
            "Metric": ["Water temperature stability", "Brewing time", "Final taste rating"],
            "Metric Type": ["Input Metric", "Process Metric", "Output Metric"],
            "Leading/Lagging": ["Leading", "Leading", "Lagging"]
        })

        st.subheader("📞 Call Center – SIPOC Overview")
        st.table({
            "Supplier": ["Client", "Internal Data Entry Team"],
            "Input": ["Client’s phone number", "CRM customer history data", "Customer’s request (information)"],
            "Process": ["Answer call and verify customer", "Access CRM system"],
            "Output": ["Problem solved", "Ticket updated in CRM"],
            "Customer": ["Client (caller)"]
        })
        st.markdown("### 📈 Metrics")
        st.table({
            "Metric": ["CRM Access Time", "Average Handle Time (AHT)", "First Call Resolution Rate", "Customer Satisfaction (CSAT)"],
            "Metric Type": ["Process Metric", "Process Metric", "Output Metric", "Outcome Metric"],
            "Leading/Lagging": ["Leading", "Leading", "Lagging", "Lagging"]
        })

    # Final Call to Action
    st.markdown("---")
    st.subheader("🌟 Final Reflection: Your Impact")
    st.markdown("""
    ✨ Now, ask yourself:
